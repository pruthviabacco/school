      ...
      >
        <div className="flex items-center gap-3 flex-1">
          ...
          <div className="text-left">
            <p className="text-sm font-bold text-[#1c3040] m-0 mb-1">{sec.label}</p>
            <div className="w-36 h-1.5 bg-[#eaf1f6] rounded-full overflow-hidden">
              <div className="h-full rounded-full" style={{ width: `${pct}%`, background: sec.color }} />
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2.5 flex-shrink-0">
          <span className="text-sm font-bold" style={{ color: sec.color }}>{fmt(sec.total)}</span>
          <span className="text-[11px] font-bold text-[#5A7A90] bg-[#eaf1f6] px-2 py-0.5 rounded-full">{pct}%</span>
          {open
            ? <ChevronUp size={15} color={sec.color} />
            : <ChevronDown size={15} color="#5A7A90" />}
        </div>
      </button>

      {/* Row body */}
      {open && (
        <div className="bg-[#f5f9fc] px-4 pl-14 pt-1 pb-3.5 border-t border-[#eaf1f6]">
          {sec.items.map((item, i) => {
            ...
            return (
              <div
                key={i}
                className="flex items-center justify-between py-2.5 border-b border-[#eaf1f6] last:border-none"
              >
                ...
                <div className="flex items-center gap-2.5">
                  <div className="w-20 h-1 bg-[#eaf1f6] rounded-full overflow-hidden">
                    <div className="h-full rounded-full opacity-60" style={{ width: `${itemPct}%`, background: sec.color }} />
                  </div>
                  <span className="text-[13px] font-bold text-[#1c3040] min-w-[80px] text-right">{fmt(item.amount)}</span>
                </div>
              </div>
            );
          })}
          <div
            className="flex justify-between mt-2.5 px-3.5 py-2.5 rounded-lg text-[13px] font-bold"
...
export default function Expense() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#d8e8f0] via-[#c8dce9] to-[#b8cfe0]">
      ...
      {/* ── Page Header ── */}
      <div className="bg-gradient-to-br from-[#1c3040] to-[#2b4557] px-8 py-5 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-white/[0.14] border border-white/[0.22] flex items-center justify-center">
            <TrendingDown size={18} color="#fff" />
          </div>
          <div>
            <h1 className="text-lg font-bold text-white m-0">Expense Details</h1>
            <p className="text-[12px] text-white/55 m-0">All categories · Click any row to expand</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          {totalExpenses > 0 && (
            <div className="text-right">
              <p className="text-[10px] text-white/50 uppercase tracking-widest m-0">Total Expenses</p>
              <p className="text-lg font-bold text-white m-0">{fmt(totalExpenses)}</p>
            </div>
          )}
          <button
            className="flex items-center gap-2 bg-white/[0.14] border border-white/[0.22] text-white rounded-xl px-4 py-2.5 text-[13.5px] font-bold hover:bg-white/[0.22] transition-all"
      ...
      <div className="p-6 max-w-5xl mx-auto">
        ...
        {!loading && mergedSections.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-[#5A7A90]">
            ...
            <p className="text-sm opacity-70 mb-5">Start by adding your first expense category.</p>
            <button
        ...
        {!loading && mergedSections.length > 0 && (
          <>
            ...
            {/* Summary footer card */}
            <div className="mt-5 p-5 bg-white/80 rounded-2xl shadow-md border border-[#3c5d74]/[0.12]">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <p className="text-[11px] font-bold text-[#3c5d74] uppercase tracking-[.7px] m-0">Grand Total — All Expenses</p>
                  <p className="text-[26px] font-bold text-[#1c3040] m-0 mt-0.5">{fmt(totalExpenses)}</p>
                </div>
